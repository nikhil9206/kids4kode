...
// const keys_camp = (Object.keys(testimonials))
var start_camp = 0
var camps = []
...
camps.push(
    new Camp(2, 'Python 2', 
             ...
             'Learn about while and for loops, more about the print() and input() and more!',
...
camps.push(
    new Camp(3, 'Python 3', 
             'Expand further on your Python knowledge!', 
...
function moveLeftCamp() {
    ...
    start_camp--
    updateCamp()
    console.log(start_camp)
}

function moveRightCamp() {
    if (camps.length - start_camp <= 3) {
        return
    }
...
function updateCamp() {
    ...
    for (let i = start_camp; i < start_camp + 3; i++) {
        tab_titles[i - start_camp].innerHTML = camps[i].get_name()
        tab_slogans[i - start_camp].innerHTML = camps[i].get_slogan()
        tab_descs[i - start_camp].innerHTML = camps[i].get_mini_desc()
        tab_skills[i - start_camp].innerHTML = camps[i].get_info_bar()['Skill Level']
        tab_times[i - start_camp].innerHTML = camps[i].get_info_bar()['Course Time']
        
        tabs[i - start_camp].setAttribute('onclick', `changeOver(${camps[i].get_id()})`)
    }
